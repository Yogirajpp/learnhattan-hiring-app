...
export const createUser = async (userData, session = null) => {
    try {
        ...
        if (existingUser) throw new CustomError(`User with email ${existingUser.email} already exists`, 400);
 ...
 */
export const getAllUsers = async (options = { filters: {}, pagination: {}, sort: "DESC" }) => {
    try {
        ...
        return { data: users, totalCount, page: pagination.page, limit: pagination.limit };
    } catch (error) {
...
export const getUserById = async (userId, returnPassword = false) => {
    try {
        const user = returnPassword ? await User.findById(userId) : await User.findById(userId).select('-password');
        return user;
    } catch (error) {
...
export const getUserByEmail = async (email, returnPassword = false) => {
    try {
        const user = returnPassword ? await User.findOne({ email }) : await User.findOne({ email }).select('-password');
        return user;
    } catch (error) {
...
export const updateUser = async (userId, updateData, session = null) => {
    try {
        const updatedUser = await User.findByIdAndUpdate(userId, updateData, session ? { session, new: true, select: '-password' } : { new: true, select: '-password' });
...
export const findAndUpdateUser = async (email, updateData, session = null) => {
    try {
        const updatedUser = await User.findOneAndUpdate({ email }, updateData, session ? { session, new: true, upsert: true, select: '-password' } : { new: true, upsert: true, select: '-password' });
        if (!updatedUser) throw new CustomError(`User ${email} not found`, 404);
        return updatedUser;
    } catch (error) {
        if (error instanceof CustomError) throw error;
        throw new CustomError(`Failed to update user by email ${email}`, 500, error);
    }
};
